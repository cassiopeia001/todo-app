...
export default function Footer(){
    ...
    return (
        <footer className={`grid grid-cols-2 grid-rows-2 gap-y-3 md:grid-cols-3 md:grid-rows-1 border-t-[1px] ${darkMode ? "border-t-Very-Dark-Grayish-Blue-dark text-Darker-Grayish-Blue" : "border-t-Light-Grayish-Blue-dark text-Dark-Grayish-Blue "} transition-colors duration-300`}>
            ...
            <div className={`row-start-2 col-start-1 col-span-2 flex items-center justify-center gap-5 rounded-md py-3 font-bold md:row-start-1 md:col-start-2 md:col-span-1 md:pt-3 md:py-0 md:pb-3 md:rounded-none ${darkMode? "bg-Very-Dark-Desaturated-Blue": "bg-white"}`}>
                ...
                <button className={`cursor-pointer ${darkMode ? "hover:text-Light-Grayish-Blue-dark" :"hover:text-Very-Dark-Grayish-Blue"} ${currentTask==='all'? "text-bright-blue":""} transition-colors duration-300`}
                        onClick={handleClick}
                        role="tab"
                        aria-selected={currentTask==='all'}>All</button>
                <button className={`cursor-pointer ${darkMode ? "hover:text-Light-Grayish-Blue-dark" :"hover:text-Very-Dark-Grayish-Blue"} ${currentTask==='active'? "text-bright-blue":""} transition-colors duration-300`}
                        onClick={handleClick}
                        role="tab"
                        aria-selected={currentTask==='active'}>Active</button>
                <button className={`cursor-pointer ${darkMode ? "hover:text-Light-Grayish-Blue-dark" :"hover:text-Very-Dark-Grayish-Blue"} ${currentTask==='completed'? "text-bright-blue":""} transition-colors duration-300`}
                        onClick={handleClick}
                        role="tab"
            ...
            <div className={`row-start-1 col-start-2 text-end rounded-br-md pr-6 py-3 md:row-start-1 md:col-start-3 ${darkMode? "bg-Very-Dark-Desaturated-Blue": "bg-white"}`}>
                <button className={`cursor-pointer text-base ${darkMode ? "hover:text-Light-Grayish-Blue-dark" :"hover:text-Very-Dark-Grayish-Blue"} transition-colors duration-300`}
